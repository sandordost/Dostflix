...
private slots:
    void exposesStableQmlRoles()
    {
        ...
        QCOMPARE(model.roleNames().value(MovieListModel::RatingRole), QByteArray("rating"));
        QCOMPARE(model.titleAt(0), QStringLiteral("Arrival"));
        QVERIFY(model.titleAt(8).isEmpty());
    }
